...
export const viewProperties = (id) => async (dispatch) => {
  console.log(" property param id in action method", id);
  // console.log(" Umni param action in action method", action);

  const data = await axios.get(
    `${ENV.baseUrl}/properties/get/${id}`
    // http://localhost:8080/v1/front/branch/get/4
    //
  );
  console.log("data of view property in action", data);
  dispatch({
