...
export function Currency() {
  ...
  const handleChange = (e) => {
    ...
    if (name === "iso" && value === "") {
      errors[name] = "your osp field is empty";
    }
    if (name === "name" && value === "") {
      errors[name] = "your name field is empty";
    }
    if (name === "exRate" && value === "") {
      errors[name] = "your exRate field is empty";
    }
    if (name === "status" && value === "") {
      errors[name] = "your status field is empty";
    }
  };
  ...
  const validate = (values) => {
    ...
    if (values.iso === "") {
      errors.iso = "please enter your iso";
    }
    if (values.name === "") {
      errors.name = "please enter your name  ";
    }
    if (values.exRate === "") {
      errors.exRate = "please enter your exRate  ";
    }
    if (values.status === "") {
  ...
  const onConfirmation = async () => {
    ...
    const data = await axios.delete(
      `${ENV.baseUrl}/currencies/delete/${params.id}`
    );
  ...
  const handleDelete = (id) => {
    ...
    setShowModal(true);
    let itemId = id;
    navigate(`/dashboard/CurrencyManagement/3/${itemId}`);
  };
  return (
    <>
      {isLoading && <FullPageLoader />}
            ...
            <form onSubmit={handleSubmit}>
              <div className="mt-4 mb-6 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                <div>
                  ...
                  <input
                    ...
                    // required
                  />
                  <p className="text-red-500">{formErrors.iso}</p>
                </div>
                <div>
                  <label className="mb-2 block text-sm font-semibold text-[#333333]">
                    Currency Name
                ...
                <div>
                  ...
                  <input
                    ...
                    // required
                  />
                  <p className="text-red-500">{formErrors.exRate}</p>
                </div>
                <div>
                  <label className="mb-2 block text-sm font-semibold text-[#333333]">
                    Status
              ...
              {isViewMode ? (
                <Button
                  className="rounded-[15px]  bg-[#280559]"
                  type="submit"
                  onClick={() => navigate("/dashboard/CurrencyManagement/*")}
                >
                  <div className="flex flex-row items-center justify-center">
                    <img src={saveIcon} alt="..." />
                    <p className="p-1 px-[11px] text-base font-medium normal-case text-white">
